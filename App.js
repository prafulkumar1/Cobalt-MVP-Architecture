...
useEffect(() => {
  ...
  const [profitCenterName, setProfitCenterName] = useState("");
   ...
   useEffect(() => {
    const fetchProfitCenterName = async () => {
      const profitCenterItem = await AsyncStorage.getItem("profit_center");
      if (profitCenterItem !== null) {
        const parsedItem = JSON.parse(profitCenterItem);
        setProfitCenterName(parsedItem?.LocationName || "");
      }
    };
    fetchProfitCenterName();
  }, []);
          ...
          <NavigationContainer>
            <Stack.Navigator
              ...
              screenOptions={({ route, navigation }) => ({
                ...
                ...UI.CbHeaderTitle("HeaderTitle",route.name === "MenuOrder" ? profitCenterName : route.name),
                headerRight: () => {
